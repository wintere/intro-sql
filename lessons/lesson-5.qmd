...
and have the same meaning, doesn't mean they are good candidates for joins.
Foreign key relationships make for good joins because you can make guarantees
about the shape and cardinality (the number of rows) in your results because
primary keys correspond to at most one row in their respective tables.
...
FROM InvoiceLine JOIN Track ON Track.UnitPrice = InvoiceLine.UnitPrice
ORDER BY Track.Name
LIMIT 5;
```
| Name | UnitPrice |
...
This query produces an enormous output, and almost all of it is meaningless. 
...
A more useful query will take advantage of the foreign key relationship between the 
Track and InvoiceLine tables, joining on `TrackId` instead. The following
query gives us, alphabetized by track name, each `InvoiceId` on which a `Track` 
was purchased.

```sql
SELECT Track.Name, InvoiceLine.UnitPrice, InvoiceId
